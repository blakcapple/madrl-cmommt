...
import time
import numpy as np 
from algos.policy.acmommt import ACMOMMT
import os
...
def test_acmommt(args_tuple):
    ...
    agents = []
    for _ in range(observer_num):
        policy = ACMOMMT(all_config.use_matrix)
    ...
    for episode in range(test_episode):
        ...
        while not done:
            ...
            local_matrix = env.local_matrix
            dict_comm = {'pursuer_observe_evader_dict': pursuer_observe_evader_dict,
                            'friend_observe_dict': friend_observe_dict,
                            'agents': env.env_core.agents}
            actions = []
            for agent_idx, agent in enumerate(env.env_core.agents):
                if agent.group == 1:
                    continue
                dict_comm['local_matrix'] = local_matrix[agent_idx]
                action = agents[agent_idx].find_next_action(dict_comm, agent)
