import numpy as np
from algos.utils.segment_tree import MinSegmentTree, SumSegmentTree
import random
...
class MultiAgentReplayBuffer:
    ...
    def __init__(self, capacity, batch_size, observation_space:spaces.Dict, n_robot, action_shape, n_step=1, share_reward=False):
        ...
        for key, val in observation_space.items():
            self.state_buf[key] = np.zeros((capacity, *val.shape), dtype=val.dtype.name)
            self.state2_buf[key] = np.zeros((capacity, *val.shape), dtype=val.dtype.name)
        if share_reward:
            self.rew_buf = np.zeros((capacity, 1), dtype=np.float32)
        else:
            self.rew_buf = np.zeros((capacity, n_robot), dtype=np.float32)
        self.act_buf = np.zeros((capacity, n_robot, action_shape), dtype=np.float32)
    ...
    def push(self, state, action, reward, state2, done):
        ...
        self.act_buf[idx] = action.copy()
        self.rew_buf[idx] = reward.copy()
