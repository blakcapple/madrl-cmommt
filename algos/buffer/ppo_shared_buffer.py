...
import torch
import numpy as np
from utils.util import get_action_dim
...
class SharedDictReplayBuffer(object):
    ...
    def __init__(self, args, n_rollout_threads, num_agents, obs_space, act_space):
        ...
        self.obs = {
            key: np.zeros((self.episode_length+1, self.n_rollout_threads, *val.shape,),dtype=np.float32)
    ...
    def insert(self, obs, actions, action_log_probs, value_preds, rewards, masks):
        ...
        self.rewards[self.step] = np.expand_dims(rewards, axis=-1)
